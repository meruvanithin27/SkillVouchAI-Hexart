...
export const ChatBot: React.FC<ChatBotProps> = ({ isOpen, onToggle }) => {
  ...
  const generateBotResponse = async (userMessage: string): Promise<string> => {
    ...
    // Enhanced Python peer recommendations using verified peer API
    if (lowerMessage.includes('python') && (lowerMessage.includes('peer') || lowerMessage.includes('recommend') || 
        lowerMessage.includes('find') || lowerMessage.includes('show') || lowerMessage.includes('expert') ||
        lowerMessage.includes('beginner') || lowerMessage.includes('advanced') || lowerMessage.includes('student'))) {
      ...
      try {
        ...
        if (!currentUser.id) {
          return 'Please log in first to get personalized Python peer recommendations.';
        }

        // Use the peer recommendations API for Python
        const peerResponse = await fetch('/api/peer-recommendations', {
          method: 'POST',
        ...
        if (peerData.verifiedPeers && peerData.verifiedPeers.length > 0) {
          let response = `🐍 **Python Peers Found** (${peerData.verifiedPeers.length} verified):\n\n`;
          
          peerData.verifiedPeers.forEach((peer: any, index: number) => {
            response += `${index + 1}. **${peer.name}**\n`;
            response += `   • Skill Level: ${peer.skillLevel}\n`;
            response += `   • Experience: ${peer.experienceYears} years\n`;
            response += `   • Rating: ⭐ ${peer.rating}/5\n`;
            if (peer.availability.length > 0) {
              response += `   • Available: ${peer.availability.join(', ')}\n`;
            }
            response += '\n';
          });

          response += '💡 **Tip**: You can send exchange requests to these peers to start learning Python!';
          return response;
        } else {
          return '🔍 **No Python peers found**\n\nNo verified Python experts are currently available. Try:\n• Checking back later\n• Broadening your search to other programming skills\n• Verifying your own Python skills to become a peer mentor!';
        }

      } catch (error) {
        console.error('Peer recommendation error:', error);
        return 'Sorry, I encountered an error while finding Python peers. Please try again.';
      }
    }
    
    // AI SQL Query Generation - Database-only approach for other skills
    ...
    // Default response
    return 'I\'m here to help you find learning peers and verify skills! You can ask me about:\n\n• **🐍 Python Peers**: "recommend python peers", "find python experts"\n• **🔍 Other Skills**: "show SQL students", "find C experts"\n• **✅ Skill Verification**: "how to verify my skills"\n• **🧭 Platform Navigation**: "how to use the platform"\n\n💡 **Pro tip**: Try "recommend python peers" for detailed verified mentor matches!';
  };
